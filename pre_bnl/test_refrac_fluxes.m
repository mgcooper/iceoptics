...
if use_dE == 1
    ...
    % this is what BnL use
%     k           =   3/4.*(data.Qext.*rhos./rhoi); % which is Rext
%         k       =   data.Rext;
%     k           =   data.Sext;
...
if use_spec_dE
    t_dir       =   100.*fcoefs.Tfa.*layer_coefs.Tdir(1:end-1,1); %#ok<UNRCH>
...
set(gca,'YDir','reverse','XScale','log','YLim',[0 1],'XLim',[10^-2 10^2]);
% set(gca,'YDir','reverse','YLim',[0 1],'XLim',[10^-3 10^2]);
ylabel('depth [m]');
xlabel('Transmissivity [%]');

legend('net flux','downflux','upflux','T Dir','Beer''s Law', ...
