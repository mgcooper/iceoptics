...
% To pick up - mostly need to go back with a fresh mind and double check
% everything. Figure out how to initialize the upper boundary properly, how
% to set z cleanly, etc. then how to optimize vertical profiles of g, w,
% etc. as per BnL. Compare with tartes.
%%
use_dE          =   1;
use_spec_dE     =   1;
use_rho_obs     =   0; % set to zero for testing
use_rho_800     =   0;
use_rho_835     =   1;
use_rho_842     =   0;
use_rho_888     =   0;
...
plot_lambda     =   400;
zr              =   0.15; % refractive boundary at 10 cm
...
%     k           =   kext./((3.*(1-w).*(1-(w.*g))).^(1/2));
    % this is what BnL use. Note that their Qs varies from 2.002-2.27
    k           =   3/4.*data.Qext./reff; % which is Rext
%         k       =   data.Rext;
%     k           =   data.Sext;
...
elseif use_rho_custom == 1
    rhos        =   rho_custom.*ones(size(0:dz:z0));
else
    rhos        =   rhos.*ones(size(0:dz:z0));
end

% these are here so I don't have to set them within the function for
% testing
rhoi            =   917;
dmu             =   0.01;
...
% rlayer_coefs =  refraction_layer_coefficients(g,w,k,rhos,fcoefs,z0,zr,dz);
...
if use_spec_dE == 1
    t_dir       =   100.*fcoefs.Tfa.*layer_coefs.Tdir(1:end-1,1);
    t_dif       =   100.*fcoefs.Tfa.*layer_coefs.Tdif_a(1:end-1,1);
else
    t_dir       =   100.*layer_coefs.Tdir(1:end-1,1);
    t_dif       =   100.*layer_coefs.Tdif(1:end-1,1);
end
t_netflux       =   100.*fluxes.NetFlux(:,1);
t_downflux      =   100.*fluxes.Fdir_dn(:,1);
t_upflux        =   100.*fluxes.Fdir_up(:,1);
t_flux_dir      =   100.*fluxes.T12dir(:,1);
t_flux_dif      =   100.*fluxes.T12dif(:,1);

figure; 
plot(t_netflux,z); hold on;
plot(t_downflux,z);
plot(t_dir,z);
plot(t_dif,z);
plot(t_beers,z);
...
set(gca,'YDir','reverse','XScale','log','YLim',[0 1],'XLim',[10^1 10^2]);
ylabel('depth [m]');
xlabel('Transmissivity [%]');

legend('net flux','downflux','T Dir','T Dif','Beer''s Law', ...
        'Mod. Beer''s Law','Measured','Location','best');
...
legend('\delta Eddington Tdir','Beer''s Law', 'Mod. Beer''s Law','Measured', ...
    'Location','northeast');
...
xlabel('depth [m]');
ylabel('Transmissivity [%]');
legend('\delta Eddington Tdif','Beer''s Law', 'Mod. Beer''s Law','Measured', ...
    'Location','northeast');
title([int2str(plot_lambda) ' nm']);    
...
xlabel('depth [m]');
ylabel('Transmissivity [%]');
legend('\delta Eddington downflux','Beer''s Law', 'Mod. Beer''s Law','Measured', ...
    'Location','northeast');
title([int2str(plot_lambda) ' nm']);

%% Netflux
figure; 
plot(z,t_netflux);hold on;
plot(z,t_beers);
plot(z,t_beers_mod);
plot(z_meas,t_meas(ind,:),'+');
set(gca,'YScale','log','XLim',[0 1],'YLim',[10^1 10^2]);
...
xlabel('depth [m]');
ylabel('Transmissivity [%]');
legend('\delta Eddington netflux','Beer''s Law', 'Mod. Beer''s Law','Measured', ...
    'Location','northeast');
title([int2str(plot_lambda) ' nm']);

%%
test = [layer_coefs.Rdir(1:5,1) layer_coefs.Tdir(1:5,1) layer_coefs.Rdif_a(1:5,1) ...
    layer_coefs.Rdif_b(1:5,1) layer_coefs.Tdif_a(1:5,1) layer_coefs.Tdif_b(1:5,1) ...
    layer_coefs.Tdrs(1:5,1) fluxes.Fdif_dn(1:5,1) fluxes.Fdif_up(1:5,1) ...
    fluxes.Fdir_dn(1:5,1) fluxes.Fdir_up(1:5,1) t_netflux(1:5) t_downflux(1:5) ...
    t_upflux(1:5)];
%% test combine layers
% combined_coefs1     =   solution_dEdd(g,w,k,rhos,u0,z0,zr,dz);
% combined_coefs2     =   combine_layers(layer_coefs,fcoefs);
...
%% tests to figure out which k to use
